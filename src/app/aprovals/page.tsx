...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    async function fetchProposals() {
      ...
      try {
        const response = await fetch(`/api/proposals/getbymanager?managerId=${userId}`);
  ...
  useEffect(() => {
    if (!isAuthenticated && userId === null) {
      ...
      const timer = setTimeout(() => {
        if (!userId) {
          setError('Usuário não autenticado. Faça login para ver as propostas.');
          setLoading(false);
        }
      }, 500);
      ...
      <div className="space-y-8">
        ...
        <div className="text-center space-y-4 animate-fade-in-down">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Gerencie as propostas recebidas nos carros que você gerencia!
          </p>
        </div>
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-8 max-w-md text-center">
              ...
              <p className="text-amber-700">
                Não há propostas pendentes para os carros que você gerencia.
              </p>
            </div>
          </div>
        )}
